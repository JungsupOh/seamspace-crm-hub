...
> 작성: 2026-07-20 · **구현 완료·CRM 검증: 2026-07-25** · 담당 연락처: info@tebahsoft.com
>
> ✅ 백엔드 구현 완료. CRM에서 실제 mDiary API로 전 경로 검증했습니다.
>   - 미사용 → 무효화(200, is_used=true/revoked) · 재무효화 멱등(200) · 없는 코드(404) 정상
>   - 사용 중(PTINTY) → 409 거절 + `used_group` 반환, 상태 불변 정상
>   - ⚠️ 무효화 직후 `coupon_info` 조회는 수 초 지연 후 반영됨 (읽기 지연) — 연동 시 즉시 재조회 금지
> 조회 시 상태 확인을 위해 `coupon_info` API도 함께 제공됩니다 (§8 참조).
