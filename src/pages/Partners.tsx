...
import type { DealFields, ContactFields } from '@/types/airtable';
import { airtable, type AirtableRecord } from '@/lib/airtable';
                  ...
                  <div className="space-y-2 rounded-md border border-border p-3">
                      ...
                      <div className="grid grid-cols-3 gap-2 mt-1">
                        {([
                          ...
                          <button key={r.v} type="button"
                            ...
                            className={`h-auto py-1.5 text-xs rounded-md border transition-colors
                              ${inviteRole === r.v ? 'border-primary bg-primary/10 text-primary font-medium' : 'border-border text-muted-foreground hover:border-primary/40'}`}>
                            {r.label}<span className="block text-[10px] opacity-70">{r.desc}</span>
                          </button>
                        ))}
                      </div>
                    </div>
                    ...
                    <div className="flex justify-end gap-2">
                      ...
                      <Button size="sm" className="h-7 text-xs"
                        onClick={() => inviteAccount(inviteEmail, inviteName, inviteRole)}
...
function PartnerDealsSection({
  ...
  // 구매자별 플랜 입력 모드 — true면 기타(직접입력) 모드 유지
  const [buyerPlanCustomMode, setBuyerPlanCustomMode] = useState<Set<number>>(new Set());
  const isBuyerPlanCustom = (idx: number, planName: string): boolean => {
    if (buyerPlanCustomMode.has(idx)) return true;
    return !!planName && !PARTNER_PLAN_LIST.includes(planName);
  };
  ...
  // 카드 합계 (모두 계약일 기준 — 표 합계와 일치):
  // - 계약금액/수수료/정산금액: 계약일 기준 전체 합
  // - 입금액: 계약일 기준 + deposit_date(입금일)가 채워진 행만 합산
  ...
  const totalDeposited  = sumByPeriod(
    unifiedRows.filter(r => r.deposit_date && r.deposit_date.trim() !== ''),
    r => r.contract_date,
    r => r.payment_amount,
    dealPeriod, dealCustomFrom, dealCustomTo,
  );
  ...
  const openAddDialog = () => {
    ...
    setBuyerPlanCustomMode(new Set());
    setSchoolQuery('');
    setSchoolResults([]);
  ...
  return (
    <section className="space-y-4">
      ...
      {/* 실적 요약 카드 — 모두 계약일 기준. 입금액은 입금일이 채워진 행만 */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <div className="surface-card ring-container p-4">
          <p className="text-xs text-muted-foreground mb-1">입금액 <span className="text-[10px] text-muted-foreground/60">· 입금일 있는 건만</span></p>
          <p className="text-2xl font-bold tabular-nums">{totalDeposited.toLocaleString()}<span className="text-sm font-normal text-muted-foreground ml-1">원</span></p>
        </div>
      </div>
              ...
              <div className="space-y-2">
                {buyers.map((b, idx) => (
                  <div key={idx} className="border border-border rounded-md p-2.5 bg-muted/30 relative">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      <div><span className="text-[10px] text-muted-foreground">학생 수</span><AmountInput value={b.student_count} onValueChange={(n) => updateBuyer(idx, 'student_count', n)} className="h-7 text-xs" /></div>
